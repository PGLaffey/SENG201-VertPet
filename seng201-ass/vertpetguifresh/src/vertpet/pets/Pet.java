...
public class Pet {
	 ...
	 */
	public String doDailyAdjustments(){
		String events = "";
		...
		if (sickChance > Math.random() && !sick){
			sick = true;
			events = events + name + " is now sick.<br>";
		}
		else if (sick){
			...
			if (cureChance > Math.random()){
				sick = false;
				events = events + name + " is feeling better and no longer sick.<br>";
			}
		}
		if (happiness > 85){
			adjustHealth(2.5);
		...
		if (hunger > 85){
			adjustHappiness(-10);
			adjustHealth(-5);
			events = events + name + " is starving!<br>";
		}
		if (thirst > 85){
			adjustHappiness(-10);
			adjustHealth(-5);
			events = events + name + " is dehydrated!<br>";
		}		
		if (tiredness > 90){
			adjustHappiness(-10);
			adjustHealth(-5);
			events = events + name + " is exhausted!<br>";
		}
		if (happiness < 10){
			adjustHealth(-5);
			events = events + name + " is extremely sad!<br>";
		}
		if (bladder > 95){
			adjustHealth(-15);
			adjustHappiness(-15);
			events = events + name + " is desperate for the bathroom!<br>";
		}
		return events;
	}
	...
	public String getStats(){
		...
		return  "<html><br>"
				+ "Health is "+ getHealth() + "<br>"
				+ getTiredness() + "% tired<br>"
				+ getHunger() + "% hungry<br>"
				+ getThirst() + "% thirsty<br>"
				+ getHappiness() + "% happy<br>"
				+ "Bladder is " + getBladder() + "% full<br>"
				+ getName() + " is" + sickValue + "sick.</html>";	
	}
	...
	/**
	 * Puts a pet to sleep and adjust the hunger accordingly.
	 * @return If the pet successfully went to sleep.
	 */
	public boolean goToSleep(){
		if (tiredness > 50){
			adjustTiredness(-35 - Math.round(Math.random()*31));
			adjustHunger(-5 - Math.round(Math.random()*11));
			if (species.equals("Cat")){
				adjustTiredness(-15.0);
				adjustHappiness(4.3);
			}
			return true;
		}
	...
	public String readDescription(String type){
		...
		try {
			...
			reader.close();
			if (!type.equals("$G$")){
				descript = descript + "<br>" + getAllLikes(type);
			}
	 ...
	 */
	private String getAllLikes(String type){
		String output = "Likes: ";
