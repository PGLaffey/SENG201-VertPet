...
public class Shop {
	...
	private Toy[] allToys = {new BouncyBall(), new Football(), new Laser(), new Mirror()
			, new Radio(), new RunningWheel(), new TV()};
	private Food[] allFood = {new Alcohol(), new Berries(), new Carrot(), new Chocolate()
			, new CookedMeat(), new Juice(), new Kibble(), new Medicine(), new Milk()
			, new Penuts(), new RawMeat(), new Seed(), new Water()};
	...
	public int getInvAmount(String itemName, String type){
		int amount = 0;
		int count = 0;
		if (type.equals("food")){
